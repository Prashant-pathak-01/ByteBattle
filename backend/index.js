...
app.post('/getHTML',(req,res) => {
  ...
  .then(response => {
    ...
    // const problemStatement = document.querySelector('.problem-statement')?.innerHTML || '';
      ...
      const problemStatement = document.querySelector('.header')?.nextElementSibling?.innerHTML.trim() || '';

      // Extract input specification
      const inputSpecification = document.querySelector('.input-specification')?.innerHTML.trim() || '';

      // Extract output specification
      const outputSpecification = document.querySelector('.output-specification')?.innerHTML.trim() || '';
      ...
      res.json({ problemStatement,inputSpecification,outputSpecification,sampleInputs,sampleOutputs });
      // return;
  })
  .catch(error => {
  ...
  ws.on('message',async (data) => {
    ...
    if(data.type === 'join'){
      ...
      if(pendingUser){
        ...
        const filteredProblems = problems.filter(problem => problem.rating <= 1700);
      ...
      [...Array(5)].forEach((_, i) => setTimeout(() => sendMessageToAll(), i * 10000));
    }
  }); 
...
const checkLatestSubmission = async (cfHandle, randomProblem) => {
  try {
    const response = await axios.get(`https://codeforces.com/api/user.status?handle=${cfHandle}&from=1&count=1`);
    ...
    if (latestSubmission && latestSubmission.problem.contestId === randomProblem.contestId && latestSubmission.problem.index===randomProblem.index) {
      return true;
    } 
  } catch (error) {
...
const sendMessageToAll = async () => {
  for (let i = games.length - 1; i >= 0; i--) {
    ...
    try {
      const cf1Matches = await checkLatestSubmission(game.cfHandle1, game.randomProblem);
      const cf2Matches = await checkLatestSubmission(game.cfHandle2, game.randomProblem);
      ...
      for(let j = 0; j < page[game.url].length; ++j ){
        if(cf1Matches)
        page[game.url][j].send(`${game.cfHandle1} won`);
        else if(cf2Matches)
        page[game.url][j].send(`${game.cfHandle2} won`);
      }
    } catch (error) {
